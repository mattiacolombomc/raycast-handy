...
function AddWordForm({ onAdd }: { onAdd: () => void }) {
  ...
  return (
    <Form actions={<ActionPanel><Action.SubmitForm title="Add Word" onSubmit={handleSubmit} /></ActionPanel>}>
      <Form.TextField title="Word" placeholder="e.g. TypeScript" {...itemProps.word} />
    </Form>
  );
}
...
export default function ManageDictionary() {
  ...
  const load = useCallback(() => {
    try { setWords(readSettings().custom_words ?? []); }
    catch (err) {
      showToast({ style: Toast.Style.Failure, title: "Could not read dictionary",
        message: err instanceof Error ? err.message : String(err) });
  ...
  async function handleDelete(word: string) {
    try {
      ...
      await showToast({ style: Toast.Style.Failure, title: "Failed to delete word",
        message: err instanceof Error ? err.message : String(err) });
    }
  }

  const addAction = (
    <Action.Push title="Add Word" icon={Icon.Plus}
  ...
  return (
    <List isLoading={isLoading} searchBarPlaceholder="Search words...">
      {words.length === 0 && !isLoading
        ? <List.EmptyView title="No custom words" description="Add one with ⌘N"
            actions={<ActionPanel>{addAction}</ActionPanel>} />
        : words.map(word => (
          <List.Item key={word} title={word} accessories={[{ icon: Icon.Trash }]}
            actions={
              <ActionPanel>
                {addAction}
