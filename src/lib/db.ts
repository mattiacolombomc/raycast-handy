...
const SELECT_COLS = "id, file_name, timestamp, saved, title, transcription_text, post_processed_text, post_process_prompt";

function queryDb<T extends Record<string, unknown>>(dbPath: string, sql: string): T[] {
...
export function toggleSaved(id: number, dbPath = DB_PATH): void {
  ...
  execDb(dbPath, `UPDATE transcription_history SET saved = ${newVal} WHERE id = ${Number(id)}`);
}
